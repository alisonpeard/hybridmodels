...
Functions from the tutorial on Sentinel-1 change detection by Mort Canty. Link: https://developers.google.com/earth-engine/tutorials/community/detecting-changes-in-sentinel-1-imagery-pt-4
...
def det(im):
    """Calculate determinant of 2x2 diagonal covariance matrix."""
...
def pval(im_list, j, m=4.4):
    """Calculate -2logRj for im_list and returns P value and -2logRj."""
...
def p_values(im_list):
    """Pre-calculate the p-value array for a list of images."""
    im_list = ee.List(im_list)
    k = im_list.length()
...
def dmap_iter(current, prev):
    """Reclassify values in directional change maps."""
...
def filter_j(current, prev):
    """Calculate change maps; iterates over j indices of pv_arr."""
    pv = ee.Image(current)
    prev = ee.Dictionary(prev)
...
def filter_i(current, prev):
    """Arrange calculation of change maps; iterates over row-indices of pv_arr."""
...
def change_maps(im_list, median=False, alpha=0.01):
    """Calculate thematic change maps."""
    k = im_list.length()
